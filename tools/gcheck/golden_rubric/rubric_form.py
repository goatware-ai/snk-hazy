...
R100_ACTION_RE = re.compile(
    ...
    # (2026-08-31). The date token is required: an assignment with no
...
# TIER ONE IS WORTH LESS THAN THIS CODE CREDITS IT (2026-08-31).
...
# landed costs, unmet-order determinations) so hard-coding fails them outright. That task
...
#
# The second FAIL landed 2026-08-31 (another package: "a solver can
...
# Extended 2026-08-31 (platform Rubric full credit completeness check
...
# R76 (2026-08-31, Rubric full credit completeness
...
# constants" (the shape two earlier tasks cleared with) failed the bar here.
    ...
    for num, text, weight in rows:
        if weight > 0 and LIVENESS_STRICT_RE.search(text) and _R76_LOCK_RE.search(text) \
            ...
            emit("ERROR", f"C{num} [R76] prescribes plain cell references as the only live mechanism - the "
                          ...
                          "could lose 10/39', 2026-08-31). Keep the value "
...
# R128 (2026-09-14, a returned task): a positive that prescribes ONE named
...
def check_function_or_label_lock(rows):
    ...
    Since: 2026-09-14 (one rubric's criterion 15 required a COUNTA and the words 'matrix cells to re-key').
    Source: the remedy asked for on that return, 'any formula-driven count ... (COUNTA, ROWS, COUNT, or equivalent) with any label that identifies the count'.
    ...
    for num, text, weight in rows:
        ...
        if m:
            emit("ERROR", f"C{num} [R128] prescribes one spreadsheet function as the required mechanism "
                          f"(\"{m.group(0)}\") - a COUNTA requirement was returned as "
                          "over-constraint (2026-09-14) and the remedy asked for 'any formula-driven "
        ...
        if m:
            emit("ERROR", f"C{num} [R128] mandates the exact words of a label (\"{m.group(0)[:60]}\") - "
                          "'the words matrix cells to re-key beside it' was returned as "
                          "over-constraint (2026-09-14) and the remedy asked for 'any label that "
                          "identifies the count'. Describe what the label identifies, never its wording")


# R81 (2026-09-02, Rubric Quality Review, [major]
...
# "names Pine Grove Creamery" (one rubric's C5) is a company, not an owner: the verb set is the
...
def check_named_owner(rows):
    ...
    Since: 2026-09-02 (Rubric Quality Review, [major] rigidity).
    Source: the ruling that a valid alternative assignment must not lose points, even when the name sits in the prompt.
    Drift-notes: this is the lower bound. R100 (2026-09-09) is the upper bound: when the prompt never asks
    for owners or dates, a row must not score them at all. The memory wording "a named person as owner and a calendar
    date" applies only when the prompt itself asks who and by when.
    """
    for num, text, weight in rows:
        if weight < 0:
            continue
        ...
        emit("ERROR", f"C{num} [R81] fixes an action's owner to a named person (\"{m.group(0)}\"): "
                      ...
                      "for the actions and not the assignments (C23-25, "
...
# R74 (2026-08-31, pre-submission): the completeness check's THIRD
...
_R74_PROMPT_RE = re.compile(
    ...
    # 2026-08-31: the platform's completeness check FAILed on exactly
...
def check_chain_scope(rows, folder):
    ...
    emit("ERROR", f"[R74] the prompt sells a dynamic-recalculation contract but every formula "
                 ...
                 "failing the requirement that the totals update on their own (2026-08-31: "
                 "two lineage criteria at 10 of 39 and the completeness check FAILed "
...
# R25 (2026-08-21): the Rubric objectivity check also rejects a bare evaluative
...
# Magnitude descriptors (2026-08-31): the objectivity
...
HEDGE_RE = re.compile(
    ...
    # OPEN-ENDED SUBSTITUTE (pre-submission, 2026-08-25): a suggested rewrite asked for
    ...
    # places by name. A suggested wording is not exempt from this check.
    r"|\bor (?:its |an |any )?(?:equivalent|equivalents|similar|comparable)\b"
    r"|\b(?:or|and) the like\b"
    # vague-intensity hedges on a MAGNITUDE (pre-submission, 2026-08-21):
    ...
    r"|\d[\d,.]*\s+or so\b"
    # vague intensity COMPARATOR on a magnitude relation (a C23 at pre-submission,
...
def check_hedges(rows):
    ...
    One pre-submission run (2026-08-20): C24 opened "Domain reasoning holds
    ...
    for num, text, _ in rows:
        ...
        if m:
            emit("ERROR", f'C{num} [R19] hedge-like qualifier "{m.group(0)}" — the platform\'s Rubric '
                          "objectivity check FAILs the dimension on one unanchored hedge (a C24 at "
...
GRADER_INSTRUCTION_RE = re.compile(
    ...
    # verdict assigned inside the text (a pre-submission run, 2026-08-20)
    ...
    # restating figures scored elsewhere reads as grader instruction too (a
    # pre-submission run, 2026-08-20 - the negatives passed, these two positives did not)
    ...
    # the negatives' exclusion carve-out, banned by the same platform check at
    # pre-submission (2026-08-26) - see the docstring
...
def check_grader_instructions(rows):
    ...
    The platform's Rubric negative polarity check FAILED one pre-submission run
    ...
    The negatives' "... is not this defect" carve-out passed that run and was
    exempt here until a later pre-submission run (2026-08-26) FAILed the
    ...
    Extended after another pre-submission run (2026-08-20), which FAILed the same check on two
    ...
    for num, text, _ in rows:
        ...
        if m:
            emit("ERROR", f'C{num} [R18] grader-instruction wording "{m.group(0)}" — the platform\'s Rubric '
                          "negative polarity check FAILs this as scoring scaffolding (pre-submission "
                          "2026-08-20; carve-outs added at pre-submission 2026-08-26). State the fact "
...
# R57 (2026-08-25, pre-submission): the platform's near-identical
...
#
# Narrow on three counts at once, which is what keeps it off the coverage criteria asked
# for elsewhere: the quantifier has to range over ROWS (each line, every row), the
...
# R60 (2026-08-25, Agentic Rubric Quality Review): two positives equated
...
# R86 (2026-09-03, Rubric near identical criteria check FAIL): subsumption
...
# bins", one rubric's C22), so only the carry verbs count; and two rows about different
# components (the Kriegl release and the quick-ship order each carrying a quantity, another
# rubric's C17/C18) are not one requirement, so the rows must share their subject.
...
def check_filename_coverage(rows, folder):
    ...
    R83 (2026-09-02, pre-submission): the platform's Rubric
    ...
    for name in sorted(set(deliverables)):
        if name not in positive:
            emit("ERROR", f"[R83] the prompt names the deliverable {name} and no positive criterion "
                          ...
                          f"coverage check FAILs on it (2026-09-02: 'references "
...
def check_verdict_column_coverage(rows, folder):
    ...
    Since: 2026-09-14 (sound_aligned_rubric confirmed).
    Source: one golden's FLOOR TEST column read HOLDS on all eight lineup rows against
    rule 6's 18 percent floor and 12 percent cover allowance, and no criterion graded margin
    compliance or the cover assignment; the finding: "a solver who violates rule 6
    ...
    golden's own verdict column, so the gap survived.
            ...
            for r, row in enumerate(grid[:8]):
                for c, v in enumerate(row):
                    ...
                    emit("ERROR", f"[R127] {path.name} {ws.title}!{head}: the golden reduces a rule to the "
                                  ...
                                  f"names the column or a verdict word - a 2026-09-14 finding caught "
                                  f"rule 6 pricing ungraded this way ('a solver "
...
# R27 (2026-08-21, Agentic Rubric Quality Review): needs_improvement, six
...
# independent verdicts to reconcile. The review's own test is "each could pass or fail
# independently". Counting numeric assertions approximates it: one rubric's two worst
...
# R49 (2026-08-24, a hand read of one rubric): that read named thirteen
# bundled positives and R27 saw two of them, because most carried only two figures or
# none. Its test is the same as the quality review's, "one could be wrong and two
...
#     assortment ... and they come to $2,058.54"), that read's own worked example:
...
# (R34), and that read flagged positives only.
...
# its own two examples spelled their counts out ("Seven items ... $2,058.54",
...
# R54 (2026-08-24, a second atomicity read of the same rubric): that read came back on nine
# rows the first split had left carrying a second scorable turn,
...
# against its labels the five signals below catch 9 of 9 but also hit six rows it did not
# name, every one of them the same shape as a row it did (a two-sentence item row beside the
# two it flagged, a parenthetical anchor beside the one it flagged). So this is not a
...
# them one task still shipped a rubric where a colon introduced a second claim (C18, C25),
...
# those is independently right or wrong, which is exactly the earlier hand-read complaint:
...
# a negative is now one sentence like everything else. An acceptance clause
...
# R82 (2026-09-02, an ACCEPT note): the task was accepted
# but every money figure in the rubric was adjusted by hand - "the currency labels in your
        ...
        for m in _R82_NUM_RE.finditer(text):
            ...
            if not dollar:
                emit("ERROR", f"C{num} [R82] money figure {val} carries no currency mark - the "
                              "accept note on one task adjusted every one of these by hand "
                              "and asked for them to be read before submission "
...
def check_rubric_lint(folder):
    ...
    Since: 2026-08-18 (oracle runs), extended through 2026-08-26.
...
def check_form_basics(rows):
    ...
    Since: R25 2026-08-21, magnitude words 2026-08-31; R9 one rubric's C8.
    ...
    for num, text, weight in rows:
        if len(text) > 500:
            ...
            # freely; do not cite the form for it.
            emit("ERROR", f"C{num} [R1] criterion is {len(text)} chars, over this desk's "
                          "500-char limit. The form asks for exactly one thing per line, and "
                          "a criterion this long is bundling several; split it")
        for sent in split_clauses(text):
            ...
            if jm:
                emit("ERROR", f'C{num} [R25] "{jm.group(0)}" is an evaluative term with no anchor in '
                              ...
                              f'exactly this (2026-08-21, one word "consistently" sank the '
        ...
        if weight > 0 and FORMULA_DEMAND_RE.search(text) and not VALUE_ANCHOR_RE.search(text) \
            ...
            emit("ERROR", f"C{num} [R9] formula-only liveness criterion with no displayed-value anchor — "
                         "judge formula extraction flakes even on pure head-call ranges (one rubric's C8 "
...
def check_weight_bands(rows):
    ...
    Drift-notes: rewritten 2026-09-21 in the Hazy port. An earlier rule banned -1 and -2
    outright; the Hazy form states the range as a continuous -5..+5 and
...
def check_penalty_share(rows):
    ...
    Since: 2026-08-25 (a note on a returned task); non-blocking by team manager ruling 2026-09-02.
    Source: a returned task.
    Drift-notes: routed through recommend(), printed and never counted.
    """
    pos, neg = _weights(rows)
    # R61 (2026-08-25, a note on a returned task): the penalty floor. The
    # note scored the rubric's deterrence as a SHARE of what it rewards -- "total
    ...
    # which is the same lever R24 already forces (one task went +132/-16 = 12.1% to
    ...
    if pos and abs(neg) < 0.20 * pos:
        recommend(f"[R61] negatives carry {abs(neg):g} against {pos:g} of positive weight "
                  ...
                  "be the main reason a task goes back. Worth "
...
def check_completeness_share(rows, folder):
    ...
    Since: 2026-08-24 (a task FAILed at 89.3%); single-loss reading 2026-08-26; R73 2026-08-31.
    Source: the platform's Rubric full credit completeness check, which fails at about 90% retention and will not certify the line. INHERITED AND UNVERIFIED FOR HAZY (2026-09-21): that check is one of ten named post-submission evals carried over from an older desk and nothing confirms Hazy runs it. The rule is kept because the underlying defect is real - a rubric a hard-coded workbook still aces tests nothing - and because the bar is a share, so it survives the change in rubric size. Drop it if the first Hazy results show no completeness check.
    ...
    if pos and has_xlsx:
        ...
        # The bar reads "about 90%", and 0.90 as a literal cutoff passed a task at 89.3%
        ...
        if pos and (pos - strict) >= 0.85 * pos:
            emit("ERROR", f"[R24] a workbook with every figure correct but keyed by hand still "
                          ...
                          f"about 90% or more and will not certify the line either (2026-08-24: "
                          f"FAILed at 89.3%; 2026-08-21: liveness held only 10 "
                          ...
                          f"absolute positive total quoted in older notes was derived from an "
                          f"older, smaller rubric and no longer holds (gated total here "
                          f"{gated:g}, leaving "
                          f"{(pos - gated) / pos:.1%})")
        ...
        # R73 (2026-08-31, an AutoEval run): the earlier
        ...
        # own figure is inseparably asserted as derived fail that solver. The
        ...
        # gate passed a rubric whose gated tier held 3 of 39 and the task was sent back
    ...
    r"|\b(?:keep|kept|keeps|built|build) (?:the )?[a-z]+(?: [a-z]+){0,5} on formulas?\b|\bcarr(?:y|ies) through the (?:sheet|workbook)\b",
        ...
        if pos and prompt_live and (pos - (gated - strict)) >= 0.85 * pos:
            emit("ERROR", f"[R73] the prompt demands live formulas throughout and the gated tier "
                          ...
                          "hard-coded workbook still earns all 39/39', 2026-08-31; an earlier "
                          "task was the first). Attach "
        ...
        # R73 REFRESH-CHAIN variant (2026-08-31): when the prompt
        ...
        # rows themselves, an earlier task's shape ("the program side on each of the 26 catalog
        ...
        # demand and refresh/rerun language (that prompt carries neither word and its
        # 1-point per-row row stays legal). Probed 2026-08-31: fires on the earlier
        # rubric (per-row gated 0), silent on the fixed one (5).
        ...
        if pos and prompt_live and prompt_rerun:
            ...
            if per_row <= 0.10 * pos + 1:
                emit("ERROR", f"[R73] the prompt promises a rerun on refreshed inputs and the per-row "
                              ...
                              f"{(pos - per_row) / pos:.1%} (2026-08-31: 39/39 with "
        ...
        # SINGLE-LOSS variant (2026-08-26, pre-submission): the same platform
        ...
        # as 51 here; another rubric's 84 read as 89). With the +5 weight cap this pins the
        ...
        # mirror negatives drew ambiguous_negative_polarity 3/3 on an earlier task (2026-08-20),
        ...
        for num, t, w in rows:
            if w > 0 and LIVENESS_STRICT_RE.search(t) and pos - w + 1 >= 0.90 * pos:
                emit("ERROR", f"C{num} [R24] hard-coding only this criterion's figure keeps "
                              ...
                              f"completeness check FAILed a rubric on (2026-08-26, single-value "
                              ...
                              f"ambiguous_negative_polarity 3/3 (W6/W15, 2026-08-20)")


# R129 (2026-09-14, a note on a returned task): the note counted
...
# Portfolio probe 2026-09-14: the 10-of-39 shape has been accepted repeatedly
# on other tasks, so the error line sits just above it.
...
def check_implementation_share(rows, folder):
    """Formula and cell-reference criteria, strict and gated together, hold a quarter of positive weight or less when the prompt makes no live-formula demand; a larger share reads as grading implementation the prompt never asked for and failing a numerically correct static workbook.

    Since: 2026-09-14 (a note on a returned task).
    Source: that note ("14 of 39 positive points (35.9%) for formula/cell-reference implementation and tab architecture (criteria 3, 7, 20-23) that the prompt never requires, exceeding the 25% implementation cap and failing a numerically correct static-value workbook").
    Drift-notes: counts every positive matching the strict or gated liveness patterns, the same set R24 reads, so with R24's 15 percent floor the band is 16 to 25 percent: one +5 strict row at positive 33 or under with one or two gated +1 rows. The 10-of-39 two-strict shape (25.6 percent) has been accepted on other tasks, so the error fires only above that share. Silent when the prompt carries R73's live-formula trigger, where the platform's completeness check demands breadth instead. Also silent on "keep it live" and "keep the X live" (2026-09-14: a prompt asking the workbook to stay live while the controller changes numbers, whose own golden was returned for typed decisions, read as static); R73's trigger does not take that phrase, because the platform's completeness check passed that rubric at a gated tier of 4 of 39. "Keep X on formulas" takes up to six words for X (2026-09-14: "Keep the classing and the bin assignments on formulas" read as static on a prompt that asks for formulas).
    """
    pos, _ = _weights(rows)
    has_xlsx = any((folder / "solution").glob("*.xlsx")) if (folder / "solution").is_dir() else False
    ...
    if share / pos > 10 / 39 + 1e-9:
        emit("ERROR", f"[R129] formula and cell-reference criteria ({names}) hold {share:g} of {pos:g} positive "
                      f"points ({share / pos:.1%}) and the prompt never asks for live formulas: the cap on "
                      f"implementation is 25% and a rubric at 14 of 39 was returned (2026-09-14) as "
                      ...
                      f"older notes was arithmetic from an older, smaller rubric and does not survive a 20-60+ rubric")
...
def _r92_grouping_coverage(rows, folder):
    ...
    for m in _GROUPED_BY_RE.finditer(prompt.read_text(encoding="utf-8")):
        ...
        emit("ERROR", f'[R92] the prompt demands a presentation "{m.group(0)}" and no positive '
                      ...
                      "mapping check FAILs on exactly this (2026-08-24). "
...
def check_prompt_every_coverage(rows, folder):
    ...
    Since: R4 an early coverage gap found by hand; R13 a C2 on 2026-08-19; R92 2026-08-24.
    ...
    if prompt.exists() and PROMPT_EVERY_RE.search(prompt.read_text(encoding="utf-8")):
        ...
        if not comp:
            emit("ERROR", '[R4] prompt demands "every/all <items>" but no criterion pins a completeness '
                         "count — spot-checks alone let omissions pass")
        else:
            ...
            if pos_total and block_w < 0.10 * pos_total:
                emit("ERROR", f"[R13] the coverage criteria ({block}) carry {block_w:g} of {pos_total:g} "
                             ...
                             "credit completeness check' FAILs at ~90%+ retained (2026-08-19, a C2 at "
...
def check_total_twins(rows):
    ...
    Since: 2026-08-19 (one rubric's C3 against its C17).
    ...
    for amount, nums in sorted(asserted.items()):
        if len(set(nums)) > 1:
            ...
            emit("ERROR", f"[R95] {amount} is asserted as a scored total in {pair} — the platform's "
                         ...
                         "(C3 vs C17, 2026-08-19); keep the total in ONE criterion and let "
...
def rubric_info(rows):
    ...
    Since: 2026-08-25 (an oracle run, fitted exactly to three rewards).
    """
    pos, neg = _weights(rows)
    # Reading an oracle reward (2026-08-25, fitted to all three rewards
...
# R99 (2026-09-09, a note on a returned task): the four rows grading the core
...
def check_flat_weighting(rows):
    ...
    Since: 2026-09-09 (a note on a returned task).
    ...
    which is the surviving form of an older guideline's 'flat weighting' mistake.
    """
...
# R100 (2026-09-09, a note on a returned task): six action rows required "a named
# person as owner and a calendar date" on each corrective action while the prompt asked only for
# "what we change on our own dock so the same mistakes stop". The note: strip the clause
...
def check_unmandated_owner_clause(rows, folder):
    ...
    for num, text, weight in rows:
        ...
        emit("ERROR", f"C{num} [R100] scores an owner or a date on an action (\"{m.group(0)}\") while the "
                      ...
                      "clause was struck from six rows on one task (2026-09-09) so that "
...
# R101 (2026-09-09, Agentic Rubric Quality Review): the review
...
# Review forced on another task (2026-09-03: allocation results stated
...
# alone are not enough: that advance label ("total loss inside the building and the
...
def check_front_page_blocks_scored(rows, folder):
    ...
    for path in sorted(d.glob("*.xlsx")) if d.is_dir() else []:
        ...
        def close():
            ...
            # are not headline figures the rubric owes a criterion (a "SALES ORDER"
            # block, 2026-09-09)
        ...
        for heading, values, labels in blocks:
            ...
            emit("ERROR", f"[R101] the front page of {path.name} carries the block \"{heading}\" and no "
                          ...
                          "leave the whole block out and lose nothing, which is how one task "
                          "shipped an unscored advance-to-be-requested block past nine passes "
...
# R102 (2026-09-10, a note on a returned task): the prompt said the document
# is "addressed to Doris" and the golden carried "To: Doris Leidy, Controller", but no
# criterion graded the addressee, and the task was sent back for a structural
...
# R105 (2026-09-11, golden check 2/3 at 1.0): C38 read
...
# Probed 2026-09-11 across the portfolio: fires on the pre-fix
# C38 only (another rubric's C18 "opens with the cabinet and lifting corrections"
...
#
#       2026-08-19: C31 keyed 'First Order'!M43 for the $13,166.52 order total
...
# R124 (2026-09-14, Rubric near identical criteria check
...
def check_subsumption(rows):
    ...
    Since: R10 2026-08-19 (one rubric's C31/C49); R47 2026-08-24 (another's C33/C36); R57 and R60 2026-08-25;
    R86 2026-09-03.
    ...
    Drift-notes: five detectors merged into one walk over the rubric on 2026-09-11; R124 added 2026-09-14.
    """
    ...
    for num, text, weight in rows:
        ...
        # row's figure (one rubric's C22/C24: two lines' prices, 6.79 and 19.01, both right)
        ...
        for b in keys[i + 1:]:
            ...
            if a[2] <= b[4] and b[2] <= a[4] and a[3] <= b[5] and b[3] <= a[5]:
                emit("ERROR", f"C{a[0]} [R10] answer key {a[6]} overlaps C{b[0]}'s key {b[6]} — "
                             ...
                             "as redundancy by subsumption and FAILs (C31/C49, 2026-08-19); "
    ...
    for num, fs in figs.items():
        ...
        if others:
            emit("ERROR", f"C{num} [R47] states only {f}, which C{others[0]} also states — the "
                         ...
                         "as subsumed by the stronger one and FAILs (C33/C36, "
    ...
    for num in universal:
        ...
        emit("ERROR", f"C{num} [R57] asserts a derivation over EVERY row while "
                     ...
                     "(2026-08-25: \"if Criterion 8 genuinely passes, those "
    ...
    for name, fs in equated.items():
        ...
        emit("ERROR", f"[R60] \"{name}\" is equated to {len(fs)} different figures "
                     ...
                     "be right\" (2026-08-25: the open commitment at "
    ...
    for i, (n1, t1) in enumerate(per_item):
        ...
        for n2, t2 in per_item[i + 1:]:
            ...
            emit("ERROR", f"C{n1} and C{n2} [R86] both assert the presence of the same attribute per item "
                          ...
                          "identical criteria check reads as redundancy by subsumption "
    ...
    for i, (n1, t1) in enumerate(pos):
        for n2, t2 in pos[i + 1:]:
            ...
            emit("ERROR", f"C{nc} [R124] closes a set (\"{_R124_CLOSED_RE.search(closed).group(0)}\") that C{ne} already "
                          ...
                          "\"exactly the 12 item and lot combinations\" (2026-09-14). "
...
def _r27_figure_count(num, text):
    ...
    Since: 2026-08-21 (six [major] non_atomic findings).
    ...
    for m in _ATOM_FIG_RE.finditer(text):
        ...
        # because a TAG code suffix is the commonest form of it: one task
        ...
        if re.search(r"[=x\u00d7/+*]\s*$", head) or re.match(r"\s*[=x\u00d7/+*]", tail):
            continue                      # a step inside one arithmetic chain, which
                                          # the review reads as a single claim
    ...
    if len(seen) >= 4:
        emit("ERROR", f"C{num} [R27] asserts {len(seen)} separate figures "
                      ...
                      "grow (one rubric reached 50 criteria at an unchanged +98)")
    elif len(seen) == 3:
...
def _r49_bundled_subjects(num, text):
    ...
    if len(items) >= 2:
        emit("ERROR", f"C{num} [R49] names {len(items)} item codes ({', '.join(items[:4])}) "
                      "under one predicate — a hand read scores each as its own verdict "
                      "(2026-08-24: \"one could be wrong and two "
    ...
    if am and len(re.findall(r"\d{4,6}", am.group(1))) >= 2:
        ...
        emit("ERROR", f"C{num} [R49] names the accounts {accts} under one predicate — "
                      "each account's history is scored on its own "
    ...
    if cm and mm and (digits is None or float(digits)
        ...
        emit("ERROR", f"C{num} [R49] states the count \"{cm.group(0)}\" and the money figure "
                      ...
                      "worked example on one rubric's C32, 2026-08-24). "
...
def _r54_second_turn(num, text):
    ...
    if hits:
        emit("ERROR", f"C{num} [R54] carries {' and '.join(hits)} — a hand read scores each "
                      "turn separately and sent one task back twice on it "
...
def _r55_atomic_sentence(num, text, weight, grader=False):
    ...
    if hits:
        emit("ERROR", f"C{num} [R55] is not one atomic sentence: {'; '.join(hits)}. A grader "
                      ...
                      "scored - one could be wrong and two right (found twice by hand). Fold the "
...
def _r85_stacked_clauses(num, text):
    """R85 (2026-09-03, Agentic Rubric Quality Review, needs_improvement:
    ...
    if len(clauses) >= 2:
        emit("ERROR", f"C{num} [R85] stacks {len(clauses)} comma-appended clauses "
                      ...
                      "gradeable elements (2026-09-03). Keep one clause "
        ...
        emit("ERROR", f"C{num} [R85] hangs a named example ({_R85_ID_RE.search(' '.join(clauses)).group(0)}) "
                      ...
                      "(2026-09-03). Give the example its own weight-neutral row")
...
def _r105_ordering_pair(num, text):
    ...
    if pair:
        emit("ERROR", f"C{num} [R105] orders two objects at once (\"{m.group(0).strip()[:90]}\"): "
                      ...
                      "verdict for two things, which flaked 1 in 3 on one task. Name "
...
def check_atomicity(rows):
    ...
    Since: R27 2026-08-21; R49 and R54 2026-08-24 (one rubric, twice); R55 2026-08-24 (house rule);
...
# R115 (2026-09-11, a note on a returned task): the prompt's first ask was
...
def check_two_way_breakout(rows, folder):
    ...
    for m in _R115_PAIR_RE.finditer(prompt.read_text(encoding="utf-8")):
        ...
        emit("ERROR", f'[R115] the prompt asks for a figure "{m.group(0)}" and no positive criterion '
                      f"carries a breakdown cue (by, each, every, per) for both {a} and {b}: a "
                      "task was sent back on exactly this (2026-09-11, "
                      "'what we spend now by branch and vendor' graded by no row). Add one criterion "
                      "for the two-way table, anchored on its total")


# R116 (2026-09-11, the platform's Rubric
...
def check_output_totals_have_positives(rows, folder):
    ...
    Since: 2026-09-14 (a returned task).
    Source: that return's rubric coverage axis - the spring-return valuation, implemented
    ...
    only when none of a sheet's labelled totals is covered, because one task's Summary tab
    ...
    Probed 2026-09-14: fires on that task pre-fix, silent post-fix and on the exemplar.
    """
    try:
        import openpyxl
    except ImportError:
        ...
        for ws in wf.worksheets:
            ...
            if footers and not any(c for _, _, c in footers):
                ...
                emit("ERROR", f"[R126] {path.name} {ws.title}!{coord} totals a labelled output "
                              ...
                              "criterion - one task was failed on the spring-return "
    ...
    for (n, noun), nums in seen.items():
        if len(nums) > 1:
            emit("ERROR", f"C{' and C'.join(str(x) for x in nums)} [R116] both carry the completeness phrase "
                         ...
                         "first (2026-09-11, C14 and C16). Keep the count on one "
                         "row and let the other assess its own point, the ordering or the named member")


# R119 (2026-09-11, the platform's "rubric does not verify computed values" finding, BLOCKING):
...
def check_presence_only_computed(rows):
    ...
    Since: 2026-09-11 (the platform's 'rubric does not verify computed values' BLOCKING finding).
    ...
    for num, text, weight in rows:
        ...
        if _R119_PRESENCE_RE.search(text) and _R119_COMPUTED_RE.search(text) and not _R119_CHECK_RE.search(text):
            emit("ERROR", f"C{num} [R119] rewards a computed figure for being present with no expected value and "
                          "no correctness check: the platform's computed-values check BLOCKED a "
                          "task on exactly this row shape (2026-09-11). Name the total the "
                          "figures reach or the source rows they reconcile to")


# R121 (2026-09-12, Rubric Quality Review
...
def check_named_acceptable_answer(rows):
    ...
    Drift-notes: positives only; "such as" and "among them" are accepted worked-example shapes on other
    rubrics and are not matched.
    """
    for num, text, weight in rows:
        if weight <= 0:
            continue
...
# stay legal. Probed 2026-09-12 across the portfolio: fires on the
# pre-fix C44 only.
...
def check_mandated_disposition(rows):
    ...
    Since: 2026-09-12 (a note on a returned task's C44).
    Source: that note (over-constrained rubric: 'mandates one business choice where the prompt invites any
    ...
    for num, text, weight in rows:
        ...
        if _R122_VERB_RE.search(text.strip()) and _R122_ALT_RE.search(text):
            ...
            emit("ERROR", f"C{num} [R122] scores one business disposition against its alternative (\"{alt}\"): "
                          "a 2026-09-12 return read the row as penalizing a "
...
def check_closing_style_criterion(rows):
    ...
    Since: 2026-09-21 (the Hazy port). There was no such rule before.
    """
    if not rows:
        return
...
def check_hedged_figures(rows):
    ...
    $15,000", good "$15,170").
    Since: 2026-09-21 (the Hazy port).
    """
    for num, text, _ in rows:
