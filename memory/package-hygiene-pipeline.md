...
1. **`tools/office_resave.py <task-folder>`** opens every `.xlsx`/`.docx` under `inputs/` and
   ...
   the recalculation notes: on one task it moved 38 values because the build used Python's
...
2. **`fix_floats.py fix`** then `scan` at zero: Excel writes full-precision `<v>` literals, so
   ...
   one-time sweep (a task failed on 47 leftover tails the day after a "portfolio-wide" sweep).
...
- **The Openpyxl generator string is evidence, never scrubbed** (G2a ruling 2026-08-26):
  ...
  debt line was paid the same day, so A14 is fixed, never recorded as debt.
- **Package-level batch tells are a STOP, not a cleanup** (reviewer, 2026-08-22): the
  ...
  sources or retired. G2c/G2d survive a genuine Word resave portfolio-wide and are not a
...
- combined = 0.7 x detect + 0.3 x llm-only, pass >= 0.55; with llm-only at 1.00 detect must reach
  ...
  notes, docProps humanisation and tab-local micro-texture do NOT move it (six runs).
...
- 2026-09-11 (a round 5, A22): the authorship reader hard-fails an input whose duration or
