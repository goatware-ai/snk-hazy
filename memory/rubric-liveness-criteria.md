...
weight arithmetic these rules live inside), [[golden-fidelity]]. The dated narratives behind
every rule here are in docs/reference/reviewer-feedback-corpus.md Part 5.
...
- The oracle judge greps CACHED VALUES. `search_xlsx` never reads formula text, so any function
  ...
  find (deadstock C18 nested LARGE 3/3, rademacher C44 head COUNTIF 3/3). Never name a function
...
- `xlsx_formula_summary` surfaces column-dominant formulas and plain reference chains. A single
  footer cell (=COUNTIF, =SUM) is invisible to it (vondrak C24 3/3). A compound cell
  (=SUMIFS(...)+IF(...), =A53+A54, =MIN(B9,B13), =B5-B15) reads as typed to one judge in three
  (marathon C29, task 20 run 6, packaging-consolidation C-strict); a boolean-product SUMPRODUCT
...
- A judge also reads sheet-level typed-share statistics and infers "the rollup is typed" without
  opening a cell (marathon run 4), and it stalls at an arithmetic hop when tracing a chain
  (marathon run 5: R3 -> E3 =C3*D3 -> C3 SUMIFS rejected at the multiplication).
...
- A row sweep ("each of the N rows draws its quantity by cell reference") multiplies W11's sweep
  flake with the extraction misread and one misread row sinks it (kolterman C29 2/3); with no
  landing cell the judge may INVENT the formula it expected and fail the golden against its own
  guess (delivery-zone 792-row zone claim, a VLOOKUP nowhere in the workbook).
- A counterfactual ("removing an item recomputes ...") cannot be executed on a static file and
  fails (marathon run 3).
...
- **Reference chain.** The front page's figures stored as `=Tab!Cell` reading through to the tabs
  ...
  history (task 23 nine of nine, task 34, task 36).
- **Assert on the READING cell, not the link** (open-order C5): name the cell that reads through
...
- **No twins**: not a COUNTIF over a text pattern many cells match (R46, the judge reads the rows
  ...
  (task 18's 949: SUMIF on a supplier-labelled row, read through on a row labelled the same way).
- **A count anchor needs one cell that reports it**: look for a wording the data already supports
  so one COUNTIF reaches the number by itself (task 20's "the requested date stands").
- **Label the anchor in the cell's own words** (R106, 2026-09-11): "the allowance standing at exactly
  ...
  allowance used sent one run in three to the other figure (rossville refinement, 0.8913 on a
...
- When the same criterion fails twice on the same TAB, move the anchor off the tab; rewording is
  the wrong lever once the location is the problem (task 20 run 7).
...
- **At most two formula-only liveness positives** (R17): each is an independent 3/3 flake risk
  (kolterman C25/C26/C27 identical wording, one failed 2/3). Both sit at +5 because R24 needs each
  alone to cost over 10% of positive plus a one-point cushion ([[rubric-criterion-count]]).
- **Weight does not protect a row**: the 3/3 rule fails on a +1 (oskaloosa run 5 lost one +1 in
  two runs). Hunt flakes regardless of weight.
- **Drop at ONE flake when a proven 3/3 row can carry the weight** (hollenbach run 3); after TWO
  flaky runs on the same row stop rewording and change its SHAPE (marathon C29, deadstock C8).
  Three rewordings in three rounds is the same signal. When a shape fails twice on different
  instances, retire the shape everywhere in one pass (task 20 bled four rounds fixing one
  instance each).
...
- The minimal form is "the cell carrying it a formula rather than a keyed figure" (or "the cell
  ...
  hollenbach run 7 and flyer-program-review. Verify each gated anchor really is a formula.
- Only on NUMERIC cells (R109, 2026-09-11): a gated "formula rather than a keyed date" clause on a
  ...
  "keyed word" clause on a nested-IF verdict failed 1/3 read by value (rossville round 4). Drop
...
- When the prompt promises a rerun on refreshed inputs, the ROW-LEVEL chain needs per-row formula
  ...
  in the checked cells retains 39/39 (tessendorf round 3). An exhaustive-carry row is a COLUMN
...
- "Live formulas" means the DECISIONS, not the arithmetic (yankton reviewer): stock/no-stock,
  ...
  already-live cells is one of the four defects that got hollenbach rejected.
...
- A live chain makes the whole file verifiable: perturb an input and diff the downstream cells
  ...
  (oskaloosa keyed the recount hold off the SLOT column that never changes instead of the WORKED
...
- A live workbook corrects part of a reviewer finding by itself (open-order's trim reverted when
...
- The deficit is the only read-back of the entered form: when it does not land on a whole
  ...
  not (task 15 run 6, 2/74 against a file at 66). The form can also carry figures the CSV never
  held (vondrak, one variant total propagated); diff the failed rows' figures against the CSV
...
- A reviewer can describe another rubric under valid criterion numbers (six of eight oskaloosa
  points), mix CSV line numbers with criterion numbers in one report, or name a finding that
...
- 2026-09-11 (packaging-consolidation Rubric Quality Review, [major] x2, coded R117/R118): two strict rows
...
- 2026-09-12 (hartwell-price-worksheet refinement round 2, adjudication): a prompt saying "keep the math live in
...
- 2026-09-12 (weldon-bridge-plan refinement round 3, adjudication, PR9): the minimal gated clause is not enough
