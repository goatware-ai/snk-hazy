...
**The standing rule (user, 2026-08-22).** Run the originality, authorship and rubric gates **on
the task folder being built or revised**, named explicitly. Do not sweep `submissions/*` and do
not report verdicts on other tasks unless asked.

The rule was set after a send-back was answered by running the new originality and authorship
checks across the whole catalogue and reporting that every task folder failed the forensic gates
and that six prompts reused the same signature narrative moves. The user's words: "I don't want
you make this kind of review anymore in other tasks."

**Why:** the finding was solicited for the task in hand, not for the back catalogue. Turning one
send-back into a portfolio-wide failure report is unasked-for work that puts every other task in
question at once, including accepted ones, and it is the user's call how and when to raise that
with the team.

**How to apply:** `tools/originality_check.py` has had its catalogue-wide mode removed for this
reason and now requires a folder argument. If a check happens to reveal something about another
task, note it once at most and leave it alone. Related: [[package-hygiene-pipeline]].
