...
sources). Narratives: docs/reference/reviewer-feedback-corpus.md Part 5.
...
- Quote money and quantities as STORED, never as formatted (R50): 10.8 not 10.80, 67.3 not
  ...
  row key, or the judge lands on the prose that spells the padded form (R94, open-order run 8).
...
- Before rewording a flaky criterion, look for the cell it needed: put the count, the period end
  date and the total where the judge lands and let the wording stand (task 20 run 1). Statement
...
- A dated criterion copies the golden's ROLE for the date (May 15 as the LAST single-receiver day,
  ...
  signature (dock-to-stock). Use the deliverable's own word for a date's role (claim deadline vs
  filing date).
...
- A criterion the reviewer disputes on a figure that is actually right shows a grading hole: add
  the criterion that states the inference (task 15's 125 under a "per register" header).
