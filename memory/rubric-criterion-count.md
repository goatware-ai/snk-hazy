...
description: "Rubric size and weight rules for Hazy: ERROR under 6 criteria, RECOMMEND under 20, NO ceiling (the form says 20-60+); any non-zero integer weight in -5..+5; the last row is the general formatting-and-style line; R24's single-loss cap on positive weight (carried over and not yet confirmed here), one atomic sentence per row (R55/R54), weight-neutral splits (R27/R49/R85), never a checker's suggested weight, rebuild rather than patch a rubric that sat between rounds"
metadata:
  type: feedback
---
...
[[rubric-negatives]]; wording in [[rubric-anchoring-and-landing]]. The older positive-at-99 and
positive-at-66 arithmetic is superseded; do not reinstate either ceiling.
...
("Weight -5 to +5", checklist item 11) with no gap around -1 and -2, so the older split band
...
**The weight ceiling is R24 (since 2026-08-26; carried over from the desk this repo was built
from and not yet confirmed here, since nothing confirms this platform runs the full credit
completeness check).**
...
seven clusters. The reference rubrics on file both landed at +39/-8, one of them at 26 rows. At 20-plus rows the +1 default is what keeps positive inside the cap; the
...
which is the bloat loop that once ran a rubric from 37 to 57 rows. R27 errors at four separate figures
...
the named rows and rebut with the cluster share (2026-09-10).
...
(one rubric's eleventh run showed nineteen gate errors from checks coded after its last entry).
...
- 2026-09-15 (a round 3, Rubric Quality Review [major] miscalibrated_weighting x2): a prompt-named side deliverable ("one sensitivity", "challenge one quantity") at +2 and +1 of 39 drew 95 and 97 percent retained; the review's own remedy was 3 or 4. Funded both to +3 inside 39 from rows whose figure other rows imply (a shipment total the per-SKU rows sum to, a shortfall equal to requested less released, a hold row the +4 decision and a negative also carry), keeping the register rows the coverage check needs. Check every prompt-named ask for a +1 or +2 sole row before submitting.
- 2026-09-22 (the form's in-app Criteria atomic FAIL, first seen on this desk): four bundle shapes pass R54/R55 and are read by hand: a relative or "with" clause stacking a serial list of checks, a figure beside its owner's name ("$4,924.99, with Ridgecrest named as that bidder"), two reasons under one verdict, and a first-plus-last location claim. Split each weight-neutrally (+2 into +1 and +1), keep the basename inside one content claim (R83), and drop a presence list sibling rows already score. A serial-list-inside-with arm was probed and fired on three rows of rubrics already passed to review, so it stays uncoded; the shapes sit in check_atomicity's Still manual note.
